...
namespace Comments.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    var callbackUrl = new Uri(@"https://gismaps.pagnet.org/CommentApp/Account/ResetPassword?userid=" + user.Id + "&code=" + code);

                    string subject = "Reset Password";
                    string body = "Please reset your password by clicking <a href=\"" + callbackUrl + "\">here</a>";
                    SendEmail.Send(subject, body, user.Email); //  (user.Email, callbackUrl, subject, body);
        ...
        public async Task<ActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            ...
            //this was added as a result of the token containing "+" chars  which when passed as a query string get changed into " " (space) in the URL
            var code = model.Code.Replace(" ", "+");
            var result = await UserManager.ResetPasswordAsync(user.Id, code, model.Password);
            //var result = await UserManager.ResetPasswordAsync(user.Id, model.Code, model.Password);//old stuff.
            if (result.Succeeded)
            {
                return RedirectToAction("ResetPasswordConfirmation", "Account");
            }
